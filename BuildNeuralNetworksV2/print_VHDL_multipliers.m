function [] = print_VHDL_multipliers(neural, normalized_list, ~,...
                          ~, N_ROWS, accuracy, RTS_bits, ...
                          ~, mean_value, std_dev, trigger_CLK,...
...
% particular architecture invokes the use of multipliers in the target Hardware. 
                      
% Store the current folder to a string
CurrentFolder = pwd;    
...
for i = 1:N_OUTPUTS
 ...
 fprintf(fid1,'LIBRARY ieee_proposed;\n');
 fprintf(fid1,'USE ieee.std_logic_1164.all;\n');
 fprintf(fid1,'USE ieee_proposed.fixed_pkg.all;\n\n');
 fprintf(fid1,'USE work.neural_library.all;\n\n');
 fprintf(fid1,'ENTITY mul IS\n');
 fprintf(fid1,'  PORT (\n');
 fprintf(fid1,'         operand_a    : IN fixedX;\n');
 fprintf(fid1,'         operand_b    : IN fixedX;\n');
 fprintf(fid1,'         EN           : IN STD_LOGIC;\n');   
 fprintf(fid1,'         CLK          : IN STD_LOGIC;\n');   
 fprintf(fid1,'         mul_res    : OUT fixedX\n');   
 fprintf(fid1,'  );\n');
 fprintf(fid1,'END ENTITY mul;\n\n');
 fprintf(fid1,'ARCHITECTURE mul OF mul IS\n');
 fprintf(fid1,'  BEGIN\n');
 fprintf(fid1,'  PROCESS(CLK) IS\n');
 fprintf(fid1,'    BEGIN\n');
 fprintf(fid1,'      IF (CLK''event AND CLK = ''%d'') THEN\n',CLK_STATE);
 fprintf(fid1,'        IF (EN = ''1'') THEN\n');
 fprintf(fid1,'          mul_res <= resize (operand_a * operand_b, UPPER_LIMIT, DOWN_LIMIT);\n');
 fprintf(fid1,'        END IF;\n');
 fprintf(fid1,'      END IF;\n');
 fprintf(fid1,'  END PROCESS;\n');
 fprintf(fid1,'END ARCHITECTURE;\n\n');
 fclose(fid1);
 ...
 fprintf(fid3,'  type weight_matrix    IS array (1 to %d,1 to %d) OF fixedX;\n',N_HIDDEN,N_INPUTS);
 ...
 fprintf(fid3,'  type ann_input_vector IS array(1 to %d) OF fixedX;\n\n',N_INPUTS);
 
 fprintf(fid3,'------------------------------------------\n'); 
 fprintf(fid3,'-- Constant declarations. Weights, biases\n');
 fprintf(fid3,'-- mean values and standard deviation\n');
 fprintf(fid3,'------------------------------------------\n\n');
 
 % Print in binary representation the values of biases for hidden nodes
 ...
 % Print in binary representation the values of biases for output nodes
 fprintf(fid3,'  CONSTANT output_bias : fixedX_vector(1 to %d) := (\n',length(neural.b{2}));
 for i = 1:length(neural.b{2})-1                                                                     
 ...
 % Print in binary representation the values of weigths for hidden nodes
 fprintf(fid3,'  CONSTANT weight      : weight_matrix  := (\n'); 
 for i = 1:N_HIDDEN-1
     fprintf(fid3,'                                             (');
     for j = 1:N_INPUTS-1
         temp4 = sfi((hidden_weights(j,i)/(std_dev(j)/0.75)), integer_bits + fraction_bits ,fraction_bits);
         number = temp4.bin;
         fprintf(fid3,'"%s",',number);
     end;
     temp4 = sfi((hidden_weights(N_INPUTS,i)/(std_dev(N_INPUTS)/0.75)), integer_bits + fraction_bits ,fraction_bits);
     number = temp4.bin;
     fprintf(fid3,'"%s"),\n',number);
 end;
 fprintf(fid3,'                                             (');
 for j = 1:N_INPUTS-1
         temp4 = sfi((hidden_weights(j,N_HIDDEN)/(std_dev(j)/0.75)), integer_bits + fraction_bits ,fraction_bits);
         number = temp4.bin;
         fprintf(fid3,'"%s",',number);
 end;   
 temp4 = sfi((hidden_weights(N_INPUTS,N_HIDDEN)/(std_dev(N_INPUTS)/0.75)), integer_bits + fraction_bits ,fraction_bits);
 number = temp4.bin;
 fprintf(fid3,'"%s"));\n',number);

     
 % Print in binary representation the mean values for each set of Inputs
 fprintf(fid3,'  CONSTANT mean : fixedX_vector(1 to %d) := (\n',N_INPUTS);
 for i = 1:N_INPUTS-1
     temp4 = sfi(mean_value(i), integer_bits + fraction_bits ,fraction_bits);
     number = temp4.bin;
     fprintf(fid3,'                                        "%s",\n',number);
 end;
 temp4 = sfi(mean_value(N_INPUTS), integer_bits + fraction_bits ,fraction_bits);
 number = temp4.bin;
 fprintf(fid3,'                                        "%s");\n\n',number);
 
 
 % Print in binary representation the standard deviation value for each set of Inputs
 fprintf(fid3,'  CONSTANT std_dev : fixedX_vector(1 to %d) := (\n',N_INPUTS);
 for i = 1:N_INPUTS-1
     temp4 = sfi(std_dev(i), integer_bits + fraction_bits ,fraction_bits);
     number = temp4.bin;
     fprintf(fid3,'                                        "%s",\n',number);
 end;
 temp4 = sfi(std_dev(N_INPUTS), integer_bits + fraction_bits ,fraction_bits);
 number = temp4.bin;
 fprintf(fid3,'                                        "%s");\n\n',number);
...
fprintf(fid3,'  CONSTANT latency    : integer := %d;\n\n',14 + ceil(log2(N_INPUTS+1)) + ceil(log2(N_HIDDEN+1)));
...
fprintf(fid3,'    end record;\n\n');
 ...
 fprintf(fid3,'  type counters       IS ARRAY(1 to %d) OF INTEGER;\n',N_OUTPUTS);
 fprintf(fid3,'  type MATRIXES       IS ARRAY(1 to %d) OF BIT_VECTOR(vector_length-1 downto 0);     -- Change this value(1000) if it cannot hold the amount of errors\n',1000);   % Change this value(1000) if it cannot hold the amount of errors
 fprintf(fid3,'  type ERROR_MATRIXES IS ARRAY(1 to %d) OF MATRIXES;\n\n',N_OUTPUTS);
...
fprintf(fid3,'COMPONENT mul IS\n');
fprintf(fid3,'  PORT (\n');
fprintf(fid3,'    operand_a    : IN fixedX;\n');
fprintf(fid3,'    operand_b    : IN fixedX;\n');
fprintf(fid3,'    EN           : IN STD_LOGIC;\n');   
fprintf(fid3,'    CLK          : IN STD_LOGIC;\n');   
fprintf(fid3,'    mul_res      : OUT fixedX\n');   
fprintf(fid3,'  );\n');
fprintf(fid3,'END COMPONENT;\n\n');
...
fprintf(fid4,'  signal mul_enable       : STD_LOGIC := ''0'';                -- This signal transfers control to the hidden Luts module\n');
fprintf(fid4,'  signal sig_enable       : STD_LOGIC := ''0'';                -- This signal transfers control to the log_sigmoid module (activation function)\n\n');
fprintf(fid4,'BEGIN\n\n');
fprintf(fid4,'-- Link each node to its corresponding multiplier entity.\n');
fprintf(fid4,'-- This is achieved with '' Generate'' command, which\n');
fprintf(fid4,'-- infers the creation of a multiplier submodule\n\n');
fprintf(fid4,'  generate_multipliers : FOR i IN 1 TO Num_Inputs GENERATE\n');
fprintf(fid4,'    mul_0   : mul port map(\n');
fprintf(fid4,'                      weight(Position,i),\n'); 
fprintf(fid4,'                      input(i),\n');
fprintf(fid4,'                      mul_enable,\n');
fprintf(fid4,'                      CLK,\n');
fprintf(fid4,'                      weight_x_input(i)\n');
fprintf(fid4,'                  );\n');
fprintf(fid4,'  END GENERATE generate_multipliers;\n\n');
fprintf(fid4,'\n');
...
fprintf(fid4,'            mul_enable <= ''0'';               -- Switch off hidden_LUT submodule\n');
...
fprintf(fid4,'                mul_enable <= ''1'';                             -- Switch on hidden_LUT submodule\n');
...
fprintf(fid6,'    mean_substraction,\n');
...
fprintf(fid6,'  signal output_inverter     : STD_LOGIC_VECTOR(1 to N_OUTPUTS) := (others => ''0'');\n');
fprintf(fid6,'  signal ann_function        : ann_modes;\n');
fprintf(fid6,'  signal fixed_input         : ann_input_vector;\n');
fprintf(fid6,'  signal no_mean_input       : ann_input_vector;\n\n');
fprintf(fid6,'BEGIN\n\n');
fprintf(fid6,'  ann0 : ann generic map (\n');
fprintf(fid6,'                           N_INPUTS,\n');
fprintf(fid6,'                           N_HIDDEN,\n');
fprintf(fid6,'                           N_OUTPUTS\n');
fprintf(fid6,'             )\n');
fprintf(fid6,'             port map    (\n');
fprintf(fid6,'                           no_mean_input,\n');
...
for i = 1:N_INPUTS
    ...
    fprintf(fid6,'          fixed_input(%d) <= to_sfixed("',i);
   for j = 1 :abs(RTS_bits(i)-integer_bits)        
        fprintf(fid6,'0');
    end;
    fprintf(fid6,'" & input.%s & "',input_map{i});
    for j =1:fraction_bits
        fprintf(fid6,'0');
    end;
    fprintf(fid6,'", UPPER_LIMIT, DOWN_LIMIT);\n');
    fprintf(fid6,'        ELSE\n');
...
fprintf(fid6,'          stage        <= mean_substraction;\n');
fprintf(fid6,'        ELSE\n');
fprintf(fid6,'          output_flag  <= ''1'';\n');
fprintf(fid6,'        END IF;\n');
fprintf(fid6,'      WHEN mean_substraction =>\n');
fprintf(fid6,'        FOR i IN 1 to 5 LOOP\n');
fprintf(fid6,'          no_mean_input(i) <= resize(fixed_input(i) - mean(i),UPPER_LIMIT, DOWN_LIMIT);\n');
fprintf(fid6,'        END LOOP;\n');
fprintf(fid6,'        enable       <= ''1'';\n');
fprintf(fid6,'        ann_function <= run;\n');
fprintf(fid6,'        stage        <= correct;\n');
fprintf(fid6,'      WHEN correct =>\n');
fprintf(fid6,'        -- Not completed yet, completed through Testbench Simulation\n\n');
...
fprintf(fid6,'          FOR i IN 1 to N_OUTPUTS LOOP\n');
fprintf(fid6,'            output(i)     <= ann_output(i) xor output_inverter(i);\n');
fprintf(fid6,'            reg_output(i) <= ann_output(i) xor output_inverter(i);\n');
fprintf(fid6,'          END LOOP;\n');
fprintf(fid6,'          output_flag <= ''1'';\n');
fprintf(fid6,'          new_input_vector <= ');
for i = 1:N_INPUTS-1
    fprintf(fid6,' hold_input.%s &',input_map{i});
end;
fprintf(fid6,' hold_input.%s;\n',input_map{N_INPUTS});
fprintf(fid6,'          enable          <= ''0'';\n');
fprintf(fid6,'          output_inverter <= (others =>''0'');\n');
...
fprintf(fid7,'  signal hybrid_output_flag : STD_LOGIC := ''1'';\n');
...
fprintf(fid7,'  signal counter            : counters := (others => 1);\n');
fprintf(fid7,'  signal ERROR_MATRIX       : ERROR_MATRIXES;\n');
...
fprintf(fid7,'\n          readline(outputfile,output_line);\n');
...
fprintf(fid7,'          END LOOP;\n');
fprintf(fid7,'          FOR i IN 1 to N_OUTPUTS LOOP\n');
fprintf(fid7,'            IF (test_output_signal(i) /= to_bit(hybrid_output(i))) THEN\n');
fprintf(fid7,'              ERROR_MATRIX(i)(counter(i)) <= to_bitvector(new_input_vector);\n');
fprintf(fid7,'              counter(i)                  <= counter(i) + 1;\n');
fprintf(fid7,'            END IF;\n');
fprintf(fid7,'          END LOOP;\n');
...
for i =1:N_INPUTS-1
    fprintf(fid7,'          write(writer,"hold_input.%s & ");\n',input_map{i});
end;
fprintf(fid7,'          write(writer,"hold_input.%s;");\n',input_map{N_INPUTS});
fprintf(fid7,'          writeline(final,writer);\n');
for i =1:N_OUTPUTS 
    fprintf(fid7,'            IF (counter(%d) > 1) THEN\n',i);
    fprintf(fid7,'              write(writer,"        CASE test_vector IS");\n');
    fprintf(fid7,'              writeline(final,writer);\n');
    fprintf(fid7,'              FOR i IN 1 to counter(%d)-1 LOOP\n',i);
    fprintf(fid7,'                write(writer,"          WHEN """);\n');
    fprintf(fid7,'                write(writer,ERROR_MATRIX(%d)(i));\n',i);
    fprintf(fid7,'                write(writer,"""  => output_inverter(%d) <= ''1'';");\n',i);
    fprintf(fid7,'                writeline(final,writer);\n');
    fprintf(fid7,'              END LOOP;\n');
    fprintf(fid7,'              write(writer,"          WHEN others => null;");\n');
    fprintf(fid7,'              writeline(final,writer);\n');
...
fprintf(fid7,'          FOR i IN 1 to N_OUTPUTS LOOP\n');
fprintf(fid7,'            misses := misses + counter(i) - 1;\n');
fprintf(fid7,'          END LOOP;\n');
...
fprintf(fid7,'          write(writer,latency);\n');
fprintf(fid7,'          writeline(report_file,writer);\n');
fprintf(fid7,'          wait_counter <= 100000;\n');
...
% Print VHDL code for hybrid implementation
fprintf(fid8,'---------------------------------------------------------\n');
...
fprintf(fid8,'--    If it does, output_inverters are set, and when the\n');
fprintf(fid8,'--    calculations of the Neural Network are finished\n');
fprintf(fid8,'--    miscalculated outputs will be inverted to provide\n');
fprintf(fid8,'--    correct outcome. If not, output is straight-forward\n');
fprintf(fid8,'--    the outcome of neural network computations.\n');
fprintf(fid8,'-- 3) drive_output: The final state of the FSM, drives\n');
fprintf(fid8,'--    the output and sets itslef in Ready mode\n\n');
fprintf(fid8,'---------------------------------------------------------\n');
...
fprintf(fid8,'    mean_substraction,\n');
fprintf(fid8,'    correct,\n');
fprintf(fid8,'    drive_output\n');
fprintf(fid8,'  );\n\n');
...
fprintf(fid8,'  signal output_inverter     : STD_LOGIC_VECTOR(1 to N_OUTPUTS) := (others => ''0'');\n');
fprintf(fid8,'  signal ann_function        : ann_modes;\n');
fprintf(fid8,'  signal fixed_input         : ann_input_vector;\n');
fprintf(fid8,'  signal no_mean_input       : ann_input_vector;\n\n');

fprintf(fid8,'BEGIN\n\n');
fprintf(fid8,'  ann0 : ann generic map (\n');
fprintf(fid8,'                  N_INPUTS,\n');
fprintf(fid8,'                  N_HIDDEN,\n');
fprintf(fid8,'                  N_OUTPUTS\n');
fprintf(fid8,'              )\n');
fprintf(fid8,'             port map    (\n');
fprintf(fid8,'                  no_mean_input,\n');
...
for i = 1:N_INPUTS
    ...
    fprintf(fid8,'          fixed_input(%d) <= to_sfixed("',i);
    for j = 1 :abs(RTS_bits(i)-integer_bits)      
        fprintf(fid8,'0');
    end;
    fprintf(fid8,'" & input.%s & "',input_map{i});
    for j =1:fraction_bits
        fprintf(fid8,'0');
    end;
    fprintf(fid8,'", UPPER_LIMIT, DOWN_LIMIT);\n');
    fprintf(fid8,'        ELSE\n');
...
if switch_enable ~= 1
    ...
    fprintf(fid8,'          WHEN others =>flag := ''1'';\n');
...
fprintf(fid8,'          stage        <= mean_substraction;\n');
fprintf(fid8,'        ELSE\n');
fprintf(fid8,'          output_flag  <= ''1'';\n');
fprintf(fid8,'        END IF;\n');
fprintf(fid8,'      WHEN mean_substraction =>\n');
fprintf(fid8,'        FOR i IN 1 to %d LOOP\n',N_INPUTS);
fprintf(fid8,'          no_mean_input(i) <= resize(fixed_input(i) - mean(i),UPPER_LIMIT, DOWN_LIMIT);\n');
fprintf(fid8,'        END LOOP;\n');
...
fprintf(fid9,'        stage        <= drive_output;\n');
fprintf(fid9,'      WHEN drive_output =>\n');
fprintf(fid9,'        IF (ann_ready = ''1'') THEN\n');
fprintf(fid9,'          FOR i IN 1 to N_OUTPUTS LOOP\n');
fprintf(fid9,'            output(i)     <= ann_output(i) xor output_inverter(i);\n');
fprintf(fid9,'            reg_output(i) <= ann_output(i) xor output_inverter(i);\n');
fprintf(fid9,'          END LOOP;\n');
fprintf(fid9,'          output_flag     <= ''1'';\n');
fprintf(fid9,'          enable          <= ''0'';\n');
fprintf(fid9,'          output_inverter <= (others =>''0'');\n');
...
% Print VHDL for final Testbench
fprintf(fid10,'----------------------------------------------\n');
fprintf(fid10,'-- This final testbench has  2 purposes:\n');
...
fprintf(fid10,'          wait_counter <= latency + 3;\n');
